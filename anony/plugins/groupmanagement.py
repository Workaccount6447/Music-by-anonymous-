...
async def on_group_message_check(_, message: types.Message):
    ...
    if text and not text.startswith("/"):
        for word in text.lower().split():
            reply = await db.get_filter(chat_id, word)
            if reply:
...
async def cmd_auth(_, message: types.Message):
    """aa: /auth — Authorise a user to use bb-level commands (mute, ban, warn…)."""
    if not await _assert_admin(message):
        return await message.reply_text("❌ Only group admins can authorise users.")
    ...
    if await is_admin(message.chat.id, target.id):
        return await message.reply_text("❌ That user is already an admin — no need to auth.")

    if await db.is_auth(message.chat.id, target.id):
        return await message.reply_text(f"ℹ️ {_mention(target)} is already authorised.")

    await db.add_auth(message.chat.id, target.id)
    await message.reply_text(
        f"✅ {_mention(target)} has been <b>authorised</b>.\n"
        f"They can now use moderation commands (mute, ban, warn, kick…)."
    )
    await _log(
        f"<b>✅ User Authorised</b>\n"
        f"Chat: <b>{message.chat.title}</b> (<code>{message.chat.id}</code>)\n"
        f"User: {_mention(target)} (<code>{target.id}</code>)\n"
        f"By: {_mention(message.from_user)}\nTime: {_now()}"
    )
...
async def cmd_unauth(_, message: types.Message):
    """aa: /unauth — Remove a user's authorisation."""
    if not await _assert_admin(message):
        return await message.reply_text("❌ Only group admins can remove authorisation.")

    target = await _get_target(message)
    if not target:
        return await message.reply_text("❌ Reply to a user or provide @username / user_id.")

    if not await db.is_auth(message.chat.id, target.id):
        return await message.reply_text(f"ℹ️ {_mention(target)} is not authorised.")

    await db.rm_auth(message.chat.id, target.id)
    await message.reply_text(f"✅ {_mention(target)} has been <b>de-authorised</b>.")
    await _log(
        f"<b>🔒 User De-authorised</b>\n"
        f"Chat: <b>{message.chat.title}</b> (<code>{message.chat.id}</code>)\n"
        f"User: {_mention(target)} (<code>{target.id}</code>)\n"
        f"By: {_mention(message.from_user)}\nTime: {_now()}"
    )
...
async def cmd_authlist(_, message: types.Message):
    """List all authorised users in this group."""
    doc = await db.db.auth.find_one({"_id": message.chat.id})
    user_ids = doc.get("user_ids", []) if doc else []
    if not user_ids:
        return await message.reply_text("📭 No authorised users in this group.")

    lines = []
    for uid in user_ids:
        try:
            u = await app.get_users(uid)
            lines.append(f"• {_mention(u)} (<code>{uid}</code>)")
        except Exception:
            lines.append(f"• <code>{uid}</code>")
...
_admincache_cooldown: dict[int, float] = {}  # user_id -> next_allowed_time


@app.on_message(filters.command(["admincache", "reload"]) & filters.group)
async def cmd_admincache(_, message: types.Message):
    """Reload the admin cache for this group. 10-minute cooldown per user."""
    import time
    user_id = message.from_user.id
    now = time.time()

    if user_id in _admincache_cooldown and now < _admincache_cooldown[user_id]:
        remaining = int(_admincache_cooldown[user_id] - now)
        return await message.reply_text(
            f"⏳ Please wait <b>{remaining}s</b> before reloading the admin cache again."
        )

    _admincache_cooldown[user_id] = now + 600  # 10 min cooldown
    sent = await message.reply_text("🔄 Reloading admin cache...")
    await db.get_admins(message.chat.id, reload=True)
    await sent.edit_text("✅ Admin cache reloaded.")
...
_tagall_tasks: dict[int, asyncio.Task] = {}
...
async def cmd_tagall(_, message: types.Message):
    ...
    if chat_id in _tagall_tasks and not _tagall_tasks[chat_id].done():
        _tagall_tasks[chat_id].cancel()
    ...
    _tagall_tasks[chat_id] = task


@app.on_callback_query(filters.regex(r"^tagall_stop_(-?\d+)$"))
async def cb_tagall_stop(_, query: types.CallbackQuery):
    """Admin-only: stop an active tagall."""
    chat_id = int(query.matches[0].group(1))

    if not await is_admin(chat_id, query.from_user.id):
        return await query.answer("❌ Only admins can stop tagall.", show_alert=True)

    task = _tagall_tasks.get(chat_id)
...
/admincache — Reload admin list (10 min cooldown)
