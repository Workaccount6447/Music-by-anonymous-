...
async def on_member_update(_, update: types.ChatMemberUpdated):
    ...
    if joined:
        ...
        raw_text = data.get("text") or getattr(config, "WELCOME_TEXT", "👋 Welcome {mention} to <b>{title}</b>!")
        photo = data.get("photo") or getattr(config, "WELCOME_PHOTO", None)
    ...
    if left:
        ...
        raw_text = data.get("text") or getattr(config, "GOODBYE_TEXT", "👋 Goodbye {first}! We'll miss you in <b>{title}</b>.")
        photo = data.get("photo") or getattr(config, "WELCOME_PHOTO", None)
        text = _format_greeting(raw_text, user, chat)
        try:
            if photo:
...
async def cmd_delall(_, message: types.Message):
    """bb: /delall — Scan recent message IDs and delete those from the target user.
    Bots cannot read chat history, so we scan a range of IDs near the current message."""
    if not await _assert_bb(message):
        return await message.reply_text("❌ Admins/authorised users only.")
    ...
    sent = await message.reply_text(
        f"🗑 Scanning and deleting messages from {_mention(target)}...\n"
        f"<i>Note: Only messages in the last ~3000 message IDs can be deleted by bots.</i>"
    )

    # Bots cannot read chat history (messages.GetHistory is user-only).
    # Strategy: scan message IDs from (current_id - 3000) to current_id,
    # try deleting them in bulk. Telegram silently skips IDs that don't belong to target.
    # This works because delete_messages accepts any IDs and skips ones the bot can't delete.
    # We use the userbot (assistant) if available to search, otherwise use ID range scan.
    chat_id = message.chat.id
    current_id = message.id
    deleted = 0

    try:
        # Try using the assistant userbot to search messages from target user
        assistant = await db.get_assistant(chat_id)
        ids_to_delete = []
        async for msg in assistant.get_chat_history(chat_id, limit=3000):
            if msg.from_user and msg.from_user.id == target.id:
                ids_to_delete.append(msg.id)
            if len(ids_to_delete) >= 100:
                try:
                    await app.delete_messages(chat_id, ids_to_delete)
        ...
        # Fallback: scan ID range and attempt bulk delete
        # Telegram will silently skip IDs not deletable by the bot
        start = max(1, current_id - 3000)
        chunk = []
        for msg_id in range(start, current_id + 1):
            chunk.append(msg_id)
            if len(chunk) >= 100:
                try:
    ...
    await sent.edit_text(f"✅ Done. Processed <b>{deleted}</b> message slots from {_mention(target)}.")
    await _log(
        f"<b>🗑 Delall</b>\nChat: <b>{message.chat.title}</b> (<code>{chat_id}</code>)\n"
        f"Target: {_mention(target)} (<code>{target.id}</code>)\n"
        f"By: {_mention(message.from_user)}\nTime: {_now()}"
    )
...
async def cb_unban(_, query: types.CallbackQuery):
    ...
    try:
        ...
        await query.message.edit_reply_markup(None)
        try:
            orig = query.message.text.html if query.message.text else (query.message.caption.html if query.message.caption else "")
...
@app.on_message(filters.group & filters.regex(r"^#(\w+)"), group=2)
...
@app.on_message(filters.group & (filters.text | filters.forwarded), group=2)
async def on_group_message_check(_, message: types.Message):
    ...
    Runs in group=2 so all command handlers (group=0) fire first.
    """
    if not message.from_user:
        return
